...
기동 직후 **시장 한 바퀴(3주기, 약 90초) 동안은 워밍업 구간**으로, 이미 지나간 공시를 발송하지 않고 기록만 합니다. 봇을 재시작할 때마다 그날의 기존 공시가 무더기로 재발송되는 것을 막기 위함입니다. 순환 조회를 하므로 한 주기만 생략해서는 부족합니다 — 첫 주기는 유가증권만 보기 때문에, 코스닥·파생상품의 기존 공시가 2~3주기에 '새 공시'로 잡혀 뒤늦게 발송됩니다.
...
30분마다 접근 여부를 확인하고, **상태가 바뀌는 순간에만** 텔레그램으로 알립니다(차단 해제 / 신규 차단). 같은 상태가 이어지면 조용하므로 알림방이 시끄러워지지 않습니다. 요청은 1회뿐이라 하루 48회로 부담이 없습니다.

확인은 **봇이 실제로 사용하는 공시 조회 요청과 동일한 방식**으로 보냅니다. 메인 페이지가 열리는 것과 공시 조회가 되는 것은 별개일 수 있어, '봇이 지금 동작할 수 있는가'를 직접 확인하는 편이 정확하기 때문입니다.

> ⚠️ **한 번의 200으로 해제를 판단하지 않습니다.** 차단이 유지되는 중에도 간헐적으로 200이 섞여 나오는 것을 실제로 확인했습니다. 그 한 번을 보고 해제 알림을 보냈다가, 봇을 재시작하자마자 다시 403을 맞았습니다. 그래서 **2회 연속 성공**해야 해제로 인정합니다(`CONFIRM_RUNS`). 30분 주기 기준으로 약 30분간 안정된 것을 확인하는 셈입니다.
